...
int main(void)
{
    ...
    int i, j, step;
    double dx, dy, dt, delta, error, Re;
    dx = 1.0/(grid-1);
    dy = 1.0/(grid-1);
    step = 1;
    dt = 0.001;
    ...
    while (error>0.00000001)
    {
        ...
        for (i=1;i<=(grid-2);i++)
        {
            for(j=1;j<=(grid-1);j++)
            {
                un[i][j] = u[i][j]- dt * (((u[i+1][j]*u[i+1][j]-u[i-1][j]*u[i-1][j])/(2.0 * dx))+((p[i+1][j]-p[i][j])/dx)+(((u[i][j]+u[i][j+1])*(v[i][j]+v[i+1][j]) -
                           (u[i][j]+u[i][j-1])*(v[i][j-1]+v[i+1][j-1]))/(4.0*dy))) + (dt/Re) * ((u[i+1][j]-2*u[i][j]+u[i-1][j])/(dx*dx) + (u[i][j+1]-2*u[i][j]+u[i][j-1])/(dy*dy));
            }
        }
        ...
        for (i=1;i<=(grid-1);i++)
        {
            for (j=1;j<=(grid-2);j++)
            {
                vn[i][j] = v[i][j] - dt * ((((v[i][j+1]*v[i][j+1]) - (v[i][j-1]*v[i][j-1])) / (2.0*dy)) + ((p[i][j+1]-p[i][j])/dy) + (((u[i][j]+u[i][j+1])*(v[i][j]+v[i+1][j]) -
                           (u[i-1][j]+u[i-1][j+1])*(v[i][j]+v[i-1][j]))/(4.0*dx)))  + (dt/Re) * ((v[i+1][j]-2*v[i][j]+v[i-1][j])/(dx*dx) + (v[i][j+1]-2*v[i][j]+v[i][j-1])/(dy*dy));
            }
        }
